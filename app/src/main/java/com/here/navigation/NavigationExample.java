...
public class NavigationExample {
    ...
    public NavigationExample(Context context, MapView mapView, TextView messageView, CameraTrackingMode cameraTrackingMode) {
        ...
        // Set initial camera tracking mode
        setCameraTrackingMode(cameraTrackingMode);
    ...
    public void stopNavigation() {
        ...
        enableDevicePositioning();
        if (cameraTrackingMode == CameraTrackingMode.DISABLED) {
            messageView.setText("No tracking.");
        }
        else {
    ...
    public void setCameraTrackingMode(@NonNull CameraTrackingMode mode) {
        if (cameraTrackingMode != mode) {
            ...
            switch (mode) {
                case DISABLED:
                    cameraBehavior = null;
                    message = "No tracking.";
                    Log.d(TAG, "No camera tracking.");
                    break;
                case FIXED:
                    cameraBehavior = new FixedCameraBehavior();
                    ((FixedCameraBehavior) cameraBehavior).setCameraTiltInDegrees(0.0);
                    ((FixedCameraBehavior) cameraBehavior).setCameraDistanceInMeters(1500.0);
                    message = "Tracking device's location.";
                    Log.d(TAG, "Fixed camera tracking.");
                    break;
                case DYNAMIC:
                    cameraBehavior = new DynamicCameraBehavior();
                    message = "Tracking device's location.";
                    Log.d(TAG, "Dynamic camera tracking.");
                    break;
                default:
            ...
            // Set camera behavior twice for force refreshing camera in the case that location doesn't change
            visualNavigator.setCameraBehavior(cameraBehavior);
        }
    }
